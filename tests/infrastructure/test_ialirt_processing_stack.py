...
def get_nlb_dns(stack_name, port, container_name):
    ...
    session = boto3.Session(profile_name="lcs")
    client = session.client("cloudformation")
    # client = boto3.client("cloudformation")
    response = client.describe_stacks(StackName=stack_name)
    output_key = f"LoadBalancerDNS{container_name}{port}"
    outputs = response["Stacks"][0]["Outputs"]
    print("Outputs from CloudFormation stack:", outputs)  # Debug information
...
def test_nlb_response():
    ...
    for stack_name, ports in stacks.items():
        for port in ports:
            ...
            assert (
                s3_response.text == f"{stack_name.lower()}_test_file.txt"
