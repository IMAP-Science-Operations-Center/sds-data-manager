"""Module for the IMAP API utilities.
"""
import argparse
import logging
import os
...
def _parse_args():
    ...
    description = (
        "This command line program invokes the downloads"
        "an file from the s3_uri, modifies it, and uploads"
        "the modified file to the same s3 uri. "
        "Example usage: python imap_api.py <s3_uri> <local_file_location>. "
    )

    s3_uri_help = (
        "The s3_uri from which to download. "
    )
...
def download(s3_uri, api_endpoint="https://api.dev.imap-mission.com"):
    ...
    url_with_parameters = api_endpoint + "/download?" + s3_uri
    ...
    # Now join the base directory with the file name
    file_name_and_path = os.path.join(base_directory, s3_uri[5:])
...
def upload(local_file_location, api_endpoint="https://api.dev.imap-mission.com"):
    ...
    put_response : requests.Response
        The response object from the PUT request to upload the file.
    """
    logging.info(f"Starting upload for file: {local_file_location}")

    remote_file_name = os.path.basename(local_file_location)

    # arbitrarily modify file name
    base, ext = remote_file_name.rsplit('.', 1)
    modified_file_name = f"{base}_mod.{ext}"
    ...
    requests.put(upload_url, data=object_text)
