...
def wipe_data_bucket(s3_client, bucket: str) -> int:
    """Delete every object (and version) from ``bucket``.
...
def run_dagster_asset_wipe(ecs_client, timeout_seconds: int = 600) -> None:
    ...
    we cannot connect to it directly. Instead we launch a one-off Fargate task
    on the Dagster cluster that reuses the daemon's task definition (and its DB
    credentials/network config) to run ``dagster asset wipe``. Wiping assets
    removes materialization/observation events but leaves the
            ...
            "containerOverrides": [
                {
                    ...
                    "command": [
                        "sh",
                        "-c",
                        "echo DELETE | dagster asset wipe --all",
                    ],
                }
            ]
        },
    ...
    if failures:
        raise RuntimeError(f"Failed to launch Dagster wipe task: {failures}")

    task_arn = run_response["tasks"][0]["taskArn"]
    _wait_for_task(ecs_client, cluster_arn, task_arn, timeout_seconds)


def _wait_for_task(
    ecs_client, cluster_arn: str, task_arn: str, timeout_seconds: int
) -> None:
        ...
        if task["lastStatus"] == "STOPPED":
            for container in task["containers"]:
                ...
                if exit_code not in (0, None):
                    raise RuntimeError(
                        f"Dagster wipe task exited with code {exit_code}: "
                        f"{container.get('reason', 'no reason given')}"
                    )
            return
        time.sleep(10)
    raise TimeoutError(
        f"Dagster wipe task {task_arn} did not stop within {timeout_seconds}s."
    )
