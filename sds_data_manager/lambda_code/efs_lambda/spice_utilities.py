...
mission_start_time = datetime(2023, 1, 1)
mission_end_time = datetime(2999, 1, 1)

def convert_spice_metadata_model_to_dict(file, base_path=''):
    spice_file_dict = {}
    spice_file_dict['file_name'] = base_path + file.file_name
...
def query_spice_metadata_database(start_time=1000, end_time=31525416070, type=None):
    ...
    SPICETable = config.active_spice_table()

    query = select(models.SPICETable.__table__)

    query = query.filter(SPICETable.min_date_j2000 <= end_time)
    query = query.filter(SPICETable.max_date_j2000 >= start_time)

    if type:
        query = query.filter(SPICETable.kernel_type == type)
...
def create_imap_metakernel(start_time=10000,
    ...
    logger.info("Loading reconstructed ephemeris files")
    recon_ephem = query_spice_metadata_database(start_time=start_time, end_time=end_time, type='recon')
    MK.load_reconstructed_ephemeris(final_ephem)

    if len(MK.gaps_in_ephemeris_data) > 0:
        logger.info("Loading preliminary reconstructed ephemeris files")
        prelim_ephem = query_spice_metadata_database(start_time, end_time, type='ephem_reconstructed', prelim_only=True)
        MK.load_reconstructed_ephemeris(prelim_ephem)

    if len(MK.gaps_in_ephemeris_data) > 0:
        logger.info("Loading predicted ephemeris files")
        predicted_ephem = query_spice_metadata_database(start_time, end_time, type='ephem_predicted')
        MK.load_predicted_ephemeris(predicted_ephem)

    logger.info("Loading final attitude files")
    final_att = query_spice_metadata_database(start_time=start_time, end_time=end_time, type='attitude_reconstructed',
                                              final_only=True)
    MK.load_final_attitude(final_att)

    if len(MK.gaps_in_attitude_data) > 0 and prelim_allowed:
        logger.info("Loading preliminary reconstructed attitude files")
        prelim_att = query_spice_metadata_database(start_time, end_time, type='attitude_reconstructed',
                                                   prelim_only=True)
        MK.load_reconstructed_attitude(prelim_att)

    if len(MK.gaps_in_attitude_data) > 0 and predict_allowed:
        logger.info("Loading predicted attitude files")
        predicted_att = query_spice_metadata_database(start_time, end_time, type='attitude_predicted')
        MK.load_predicted_attitude(predicted_att)

    logger.info(f"Returning the metakernel: {MK}")
...
class MetaKernel:
    ...
    def __init__(self, start_time, end_time):
        ...
        # Holds all files
        self.static_files = []
        self.final_ephemeris = []
        self.reconstructed_ephemeris = []
        self.predicted_ephemeris = []
        self.final_attitude = []
        self.reconstructed_attitude = []
        self.predicted_attitude = []
    ...
    def load_attitude(self, attitude_files):
        ...
        self.attitude_files.extend(final_recon_files_to_load)
    ...
    def return_spice_files_in_order_detailed(self):
        ...
        list_of_ephem = []
        list_of_ephem.extend(self.predicted_ephemeris)
        list_of_ephem.extend(self.reconstructed_ephemeris)
        list_of_ephem.extend(self.final_ephemeris)
        list_of_att = []
        list_of_att.extend(self.predicted_attitude)
        list_of_att.extend(self.reconstructed_attitude)
        list_of_att.extend(self.final_attitude)

        metakernel_files = []
        metakernel_files.extend(self.static_files)
        metakernel_files.extend(list_of_ephem)
        metakernel_files.extend(list_of_att)
    ...
    def _reformat_and_filter(self, spice_items):
        ...
        file_dict = {}
        for file_name, file_info in spice_items.items():
    ...
    def _find_best_files(self, trange, files_to_check, files_to_load):
        ...
        gap_list = []
        return_gap_list = []
        # Find the "best" file to load in by latest date and OD number
        largest_od = -1
        latest_delivery_date = -1
        best_file = None
        for file_root in files_to_check:
            if files_to_check[file_root]['OD'] > largest_od:
                largest_od = files_to_check[file_root]['OD']
                best_file = files_to_check[file_root]
            elif files_to_check[file_root]['OD'] == largest_od and files_to_check[file_root][
                'timestamp'] < latest_delivery_date:
