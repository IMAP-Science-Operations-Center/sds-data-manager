...
from orchestration import pointing_attitude, spice, spin
...
outputs_dict = {
    "glows_l1a_all": ["glows_l1a_hist", "glows_l1a_de"],
    "idex_l1a_all": [
        "idex_l1a_msg-10days",
        "idex_l1a_sci-10days",
...
class IMAPJobHandler:
    ...
    def __init__(self, asset_name, partition):
        ...
        ----------
        asset_name : str
            The asset name in the format source_data_type_descriptor.
        partition : PartitionsDefinition
            The Dagster partition definition for this job.
        """
        ...
        for dep in upstream_deps_list:
            if dep["source"] == "pointing_attitude":
                ...
                asset_name = (
                    dep["source"] + "_" + dep["data_type"] + "_" + dep["descriptor"]
                )
        ...
        self.job = define_asset_job(
            name=f"{self.asset_name}_processing_job",
        ...
        def _generic_batch_submitter(context: AssetExecutionContext):
            ...
            with db.Session() as session:
                ...
                if not already_processed:
                    ...
                    for output in self.outputs:
                        context.log.info(
                            f"Waiting for data product {output} to complete."
                        )
    ...
    def build_spice_deps_asset(self):
        ...
        def _generic_spice_maker(context):
            ...
            spice_files = spice.get_upstream_dependency_inputs_spice(
                self.spice_types, start_date, end_date
            )
    ...
    def build_spin_deps_asset(self):
        ...
        def _generic_spin_maker(context):
            ...
            spin_files = spin.get_upstream_dependency_inputs_spin(start_date, end_date)
    ...
    def build_attitude_pointing_deps_asset(self):
        ...
        def _generic_pointing_attitude_maker(context):
            ...
            pointing_attitude_files = (
                pointing_attitude.get_upstream_dependency_inputs_repoint(
                    start_date, end_date
                )
            )
        ...
        def _sensor(context: SensorEvaluationContext):
            ...
            for dep_key in deps_keys:
                ...
                for record in new_events:
                    ...
                    if (
                        not new_cursors.get(dep_name)
                        or record.storage_id > new_cursors[dep_name]
                    ):
                    ...
                    up_start, up_end = self._parse_dates_from_key(
                        upstream_partition_key
                    )
    ...
    def get_dependencies(self, context, target_start, target_end):
        ...
        for dep_key in self.deps_list:
            ...
            if not materialized_partitions:
                ...
                context.log.info(
                    f"Not enought information to process. Missing {dep_key} in range {target_start!s} to {target_end!s}"
                )
                return
                ...
                if up_start < target_end and up_end > target_start:
                    ...
                    if mat_event and mat_event[0].asset_materialization:
                        ...
                        if "file_names" in metadata:
                            ...
                            if input_type == "spin":
                                dependency_inputs.add(
                                    processing_input.SpinInput(*file_names)
                                )
                            ...
                            if input_type == "spice":
                                dependency_inputs.add(
                                    processing_input.SPICEInput(*file_names)
                                )
            if not found_dep:
